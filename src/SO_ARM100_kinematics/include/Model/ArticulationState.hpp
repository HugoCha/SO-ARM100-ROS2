...
const Vec3d& Origin() const {
	return center_.origin;
}

const Vec3d& Axis() const {
	return joint_states_.rbegin()->second->Axis();
}

double Value() const {
	return joint_states_.rbegin()->second->Value();
}
...
void SetPose( const Quaternion& rotation, const Vec3d& origin );
...
std::map< JointConstPtr, JointStatePtr > joint_states_;
