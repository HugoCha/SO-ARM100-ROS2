...
SphericalSolution EulerSolver::SolveFromRotation( const Mat3d& R_target ) const
{
	SphericalSolution out{};

	const auto& branches = model_.DecomposePhysical( R_target );

	std::vector< SphericalSolutionBranch > euler_branches( 2 );
...
SphericalSolution EulerSolver::SolveAndOptimizeFromTwoVectors(
	const Vec3d& p_tcp_local,
...
double EulerSolver::DeviationCost( const Vec3d& prefered, const Vec3d& angles ) const
{
...
double EulerSolver::LimitViolationCost( const Vec3d& angles, double violation_weight ) const
{
	double cost = 0.0;
...
double EulerSolver::SingularityCost( const Mat3d& R_canonical ) const
{
...
SphericalSolutionBranch EulerSolver::GridSearch( const CostFn& f ) const
{
...
SphericalSolutionBranch EulerSolver::FirstSolutionSearch( const CostFn& f ) const
{
...
SphericalSolutionBranch EulerSolver::GoldenSearchSection(
	const CostFn& f,
	double a,
	double b,
	double tol ) const
...
bool EulerSolver::CheckLimits( Vec3d& angles ) const
{
	bool all_in = true;
	for ( int i = 0; i < 3; ++i )
	{
